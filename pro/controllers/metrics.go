...
func MetricHandlers(r *mux.Router) {
	r.HandleFunc("/api/metrics/{network}/{nodeid}", middleware.Scope(scope.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getNodeMetrics)))).Methods(http.MethodGet)
	r.HandleFunc("/api/metrics/{network}", middleware.Scope(scope.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getNetworkNodesMetrics)))).Methods(http.MethodGet)
	r.HandleFunc("/api/metrics", middleware.Scope(scope.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getAllMetrics)))).Methods(http.MethodGet)
	r.HandleFunc("/api/metrics-ext/{network}", middleware.Scope(scope.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getNetworkExtMetrics)))).Methods(http.MethodGet)
}
