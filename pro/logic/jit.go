...
func userMustSatisfyJIT(network *schema.Network, user *schema.User) bool {
	...
	for _, gid := range network.JITUserGroupIDs {
		if _, ok := user.UserGroups.Data()[gid]; ok {
			return true
		}
	}
	return false
...
func UpdateJITUserGroupsOnNetwork(networkID string, jitUserGroupIDs []schema.UserGroupID) error {
	...
	if !network.JITEnabled {
		return errors.New("JIT is not enabled on this network")
	}
...
func GetUserJITNetworksStatus(networks []schema.Network, user *schema.User) ([]UserJITNetworkStatus, error) {
	...
	for _, network := range networks {
		status := UserJITNetworkStatus{
			...
			NetworkName:    network.Name, // Can be enhanced with network display name if available
			JITEnabled:     network.JITEnabled,
			HasAccess:      false,
...
func ExpireJITGrants() error {
	...
	for _, expiredGrant := range expiredGrants {
		...
		if err := removeUserJITNetworkAccess(expiredGrant.NetworkID, expiredGrant.UserID); err != nil {
...
func groupGrantsNetworkAdminOn(g *schema.UserGroup, netID schema.NetworkID) bool {
	if g == nil {
		return false
	}
	roles, ok := g.NetworkRoles.Data()[netID]
	if !ok {
		return false
	}
...
// groupGrantsGlobalNetworkAdmin reports whether the group grants admin access
// across all networks, via either the network-admin role under the
// all-networks scope or the dedicated global-network-admin role.
func groupGrantsGlobalNetworkAdmin(g *schema.UserGroup) bool {
	if g == nil {
		return false
	}
...
func RemoveUserJITNetworkAccess(networkID, userID string) error {
	return removeUserJITNetworkAccess(networkID, userID)
}
...
func removeUserJITNetworkAccess(networkID, userID string) error {
	var firstErr error
	if err := disconnectUserExtClients(networkID, userID); err != nil && firstErr == nil {
		firstErr = err
	}
	if err := disconnectUserHostNodes(networkID, userID); err != nil && firstErr == nil {
		firstErr = err
	}
	return firstErr
}
...
func disconnectUserHostNodes(networkID, userID string) error {
	...
	for _, schemaNode := range schemaNodes {
		...
		hostCopy := host
		go mq.PublishMqUpdatesForDeletedNode(&hostCopy, node, true)
