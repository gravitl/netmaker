...
type ServerSettings struct {
	...
	Value datatypes.JSONType[ServerSettingsData]
}
...
func (s *ServerSettings) TableName() string {
	return "server_settings"
}

func (s *ServerSettings) Upsert(ctx context.Context) error {
	return db.FromContext(ctx).Save(&s).Error
}

func (s *ServerSettings) Get(ctx context.Context) error {
	return db.FromContext(ctx).Model(&ServerSettings{}).
		Where("key = ?", s.Key).
		First(&s).
		Error
}

func (s *ServerSettings) Delete(ctx context.Context) error {
	return db.FromContext(ctx).Delete(&s).Error
}
