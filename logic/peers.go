		...
		for _, peer := range currentPeers {
			...
			if servercfg.IsPro {
				if (node.IsRelayed && node.RelayedBy != peer.ID.String()) ||
					(peer.IsRelayed && peer.RelayedBy != node.ID.String()) || isFailOverPeer || (node.InternetGwID != "" && node.InternetGwID != peer.ID.String()) || ((peer.InternetGwID != "" && peer.InternetGwID != node.ID.String()) && (node.IsRelayed && node.RelayedBy != peer.ID.String())) {
