...
func enrollmentKeyHandlers(r *mux.Router) {
	r.HandleFunc("/api/v1/enrollment-keys", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(createEnrollmentKey)))).
		Methods(http.MethodPost)
	r.HandleFunc("/api/v1/enrollment-keys", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getEnrollmentKeys)))).
		Methods(http.MethodGet)
	r.HandleFunc("/api/v2/enrollment-keys", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(listEnrollmentKeys)))).
		Methods(http.MethodGet)
	r.HandleFunc("/api/v1/enrollment-keys/network/{network}/default", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getDefaultEnrollmentKeyForNetwork)))).
		Methods(http.MethodGet)
	r.HandleFunc("/api/v1/enrollment-keys/{keyID}/regenerate-token", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(regenerateEnrollmentKeyToken)))).
		Methods(http.MethodPost)
	r.HandleFunc("/api/v1/enrollment-keys/{keyID}", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(deleteEnrollmentKey)))).
