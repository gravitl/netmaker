...
func userHandlers(r *mux.Router) {
	...
	r.HandleFunc("/api/users/adm/transfersuperadmin/{username}", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(transferSuperAdmin)))).
		Methods(http.MethodPost)
	r.HandleFunc("/api/users/adm/authenticate", authenticateUser).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}/validate-identity", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(validateUserIdentity))))).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}/auth/init-totp", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(initiateTOTPSetup))))).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}/auth/complete-totp", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(completeTOTPSetup))))).Methods(http.MethodPost)
	...
	r.HandleFunc("/api/users/{username}", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(updateUser)))).Methods(http.MethodPut)
	r.HandleFunc("/api/users/{username}", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(createUser)))).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(deleteUser)))).Methods(http.MethodDelete)
	r.HandleFunc("/api/users/{username}", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(getUser))))).Methods(http.MethodGet)
	r.HandleFunc("/api/users/{username}/enable", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(enableUserAccount)))).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}/disable", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(disableUserAccount)))).Methods(http.MethodPost)
	r.HandleFunc("/api/users/{username}/settings", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(getUserSettings))))).Methods(http.MethodGet)
	r.HandleFunc("/api/users/{username}/settings", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(updateUserSettings))))).Methods(http.MethodPut)
	r.HandleFunc("/api/v1/users", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatchOrAdmin(http.HandlerFunc(getUserV1))))).Methods(http.MethodGet)
	r.HandleFunc("/api/users", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getUsers)))).Methods(http.MethodGet)
	r.HandleFunc("/api/v2/users", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(listUsers)))).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/users/bulk", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(bulkDeleteUsers)))).Methods(http.MethodDelete)
	r.HandleFunc("/api/v1/users/bulk/status", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(bulkUpdateUserStatus)))).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/users/roles", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(ListRoles)))).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/users/access_token", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(createUserAccessToken)))).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/users/access_token", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(getUserAccessTokens)))).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/users/access_token", Scope(db.TenantScope, logic.SecurityCheck(true, http.HandlerFunc(deleteUserAccessTokens)))).Methods(http.MethodDelete)
	r.HandleFunc("/api/v1/users/logout", Scope(db.TenantScope, logic.SecurityCheck(false, logic.ContinueIfUserMatch(http.HandlerFunc(logout))))).Methods(http.MethodPost)
}
...
func createUserAccessToken(w http.ResponseWriter, r *http.Request) {
	...
	if caller.Username != user.Username && caller.PlatformRoleID != schema.SuperAdminRole {
		if caller.PlatformRoleID == schema.AdminRole {
	...
	logic.LogEvent(&models.Event{
		...
		Target: models.Subject{
			ID:   req.ID,
			Name: req.Name,
			Type: schema.UserAccessTokenSub,
			Info: req,
		},
	...
	if !user.IsMFAEnabled {
		if val := request.Header.Get("From-Ui"); val == "true" {
			...
			logic.LogEvent(&models.Event{
				...
				TriggeredBy: user.Username,
				Target: models.Subject{
					ID:   schema.ClientAppSub.String(),
					Name: schema.ClientAppSub.String(),
					Type: schema.ClientAppSub,
				},
...
func createUser(w http.ResponseWriter, r *http.Request) {
	...
	if caller.PlatformRoleID != schema.SuperAdminRole && user.PlatformRoleID == schema.AdminRole {
	...
	if !servercfg.IsPro && user.PlatformRoleID != schema.AdminRole {
...
func updateUser(w http.ResponseWriter, r *http.Request) {
	...
	if user.PlatformRoleID != userchange.PlatformRoleID && userchange.PlatformRoleID == schema.SuperAdminRole {
...
func logout(w http.ResponseWriter, r *http.Request) {
	...
	var target schema.SubjectType
	if val := r.Header.Get("From-Ui"); val == "true" {
		target = schema.DashboardSub
	} else {
